...
export class UsuarioController{
    ...
    @ApiOperation({description:'Traer usuarios', summary:'Encontrar usuarios'})
    @Get()
    ...
    @ApiOperation({description:'Actualizar usuario', summary:'Actualizar usuario'})
    @Put(':id')
    async update(@Param('id', ParseIntPipe) id:number, @Body() updateUsuario: UpdateUsuarioDto){
        const response = await this.usuarioService.update( id , updateUsuario);
        return response; 
    }

    @ApiOperation({description:'Eliminar usuario', summary:'Eliminar usuario'})
    @Delete(':id')
